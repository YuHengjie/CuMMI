...
# %%
df = pd.read_csv("../data/all_rpa_fill.csv",keep_default_na=False, na_values=[''])
df
...
# %%
# Specify the column range to encode 
feature_columns = df_x.columns[1:38]
feature_columns
...
for col in feature_columns:
    ...
    df_col = df_x.copy()
    df_col[col] = 'Unknown'
    texts = df_col.apply(row_to_structured_text, axis=1).tolist()
    ...
    np.save(f"./fill/text_embeddings_{cleaned_col}.npy", embeddings)

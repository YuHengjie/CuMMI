...
# %%
mask_external_plasma = df['Incubation protein source'].str.contains(
    'plasma', case=False, na=False
)
mask_external_human = df['Protein source organism'].str.contains(
    'human', case=False, na=False
)

df_plasma_human = df[mask_external_plasma & mask_external_human]
df_plasma_human
...
# %%
df_plasma_human = df_plasma_human[~df_plasma_human['Label'].isin(problematic_labels)]
print(f"筛选到 {len(df_plasma_human)} 条记录。")

# %%
df_plasma_human[['Core', 'Surface modification']].value_counts()

# %%
df_plasma_human['DOI'] = (
    df_plasma_human['Label']
    .astype(str)                      # 保险：防止非字符串
    .str.split('\\')                  # 按路径分割
    .apply(lambda parts: next(
        (p for p in parts if p.startswith('10.')),
        None
    ))
)
df_plasma_human['DOI'].unique()

# %%
# 1. 基础组合统计
combo_counts = (
    df_plasma_human
    .groupby(['Core', 'Surface modification'])
    .size()
    .reset_index(name='count')
)

# 2. 统计每个 Core 对应的独特 DOI 数
core_doi_counts = (
    df_plasma_human
    .groupby('Core')['DOI']
    .nunique()
    .reset_index(name='core_unique_doi_count')
)

# 3. 统计每个 Surface modification 对应的独特 DOI 数
surf_doi_counts = (
    df_plasma_human
    .groupby('Surface modification')['DOI']
    .nunique()
    .reset_index(name='surf_unique_doi_count')
)

# 4. 合并回 combo_counts
combo_counts = (
    combo_counts
    .merge(core_doi_counts, on='Core', how='left')
    .merge(surf_doi_counts, on='Surface modification', how='left')
)

combo_counts

# %%
df_plasma_human = df_plasma_human.drop(columns=['DOI'])
df_plasma_human

# %%
# 1. rule: 筛选满足要求的组合
rule_mask = (
    (combo_counts['core_unique_doi_count'] > 1) &
    (combo_counts['count'] < 40000)
)
rule_mask.sum()

# %%
# 2. 根据 rule 筛选组合
filtered_combos = combo_counts[rule_mask][['Core', 'Surface modification']]
# 3. 从这些组合里随机抽取 15%
repre_select_rule = filtered_combos.sample(frac=0.20, random_state=42)
repre_select_rule

# %%
# 组合转换成 tuple 列表
target_pairs = list(repre_select_rule.itertuples(index=False, name=None))

# 在 df_plasma_human 中筛选
mask = df_plasma_human[['Core', 'Surface modification']].apply(
    tuple, axis=1
).isin(target_pairs)

df_nano_external_plasma_human = df_plasma_human[mask]
df_nano_external_plasma_human

# %%
df_nano_external_plasma_human['Protein corona composition'].sum()

# %%
df_nano_external_plasma_human['Incubation protein source'].unique()

# %%
df_nano_external_plasma_human['Protein source organism'].unique()

# %%
df_nano_external_plasma_human = df_nano_external_plasma_human[df_nano_external_plasma_human['Imputation confidence']==1]
print(df_nano_external_plasma_human['Protein source organism'].unique())
print(f"筛选到 {len(df_nano_external_plasma_human)} 条记录。")

# %%
df_nano_external_plasma_human.to_csv("data/nano_external_plasma_human.csv", index=False)


# %%
# 获取 df_nano_external_plasma_human 中唯一的 (Core, Surface modification) 组合
unique_combos_nano = df_nano_external_plasma_human[['Core', 'Surface modification']].drop_duplicates()

# 使用 'Core' 和 'Surface modification' 组合来判断差异
mask = ~df[['Core', 'Surface modification']].apply(
    tuple, axis=1
).isin(
    unique_combos_nano.apply(tuple, axis=1)  # 使用独特值进行比较
)
mask

...
# %%
valid_accessions = counts[(counts > 5) & (counts < 200)].index
valid_accessions

# %%
sampled_accessions = valid_accessions.to_series().sample(frac=0.25, random_state=42)
sampled_accessions

# %%
df_pro_external_plasma_human = df_plasma_human[df_plasma_human['Accession'].isin(sampled_accessions)]
df_pro_external_plasma_human

# %%
df_pro_external_plasma_human['Protein corona composition'].sum()

# %%
df_pro_external_plasma_human['Incubation protein source'].unique()

# %%
df_pro_external_plasma_human['Protein source organism'].unique()

# %%
df_pro_external_plasma_human = df_pro_external_plasma_human[df_pro_external_plasma_human['Imputation confidence']==1]
print(df_pro_external_plasma_human['Protein source organism'].unique())
print(f"筛选到 {len(df_pro_external_plasma_human)} 条记录。")

# %%
df_pro_external_plasma_human.to_csv("data/protein_external_plasma_human.csv", index=False)



# %%
mask = ~df['Accession'].isin(sampled_accessions)
mask

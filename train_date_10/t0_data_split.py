...
# %%
external_doi_list = [ '10.1021_acsami.4c21207',
    '10.1021_acs.jproteome.4c00545',
    '10.3762_bjnano.15.100',
    '10.1016_j.jconrel.2024.07.044',
    '10.1021_acs.chemmater.4c01124',
                ]

regex_escaped_dois = [re.escape(doi) for doi in external_doi_list]
doi_pattern = '|'.join(regex_escaped_dois)

# 2. 找出所有【包含】任一 DOI 的行
# regex=True 是 str.contains 的默认值
mask_contains = df['Label'].str.contains(doi_pattern, case=False, na=False)

df_date_external = df[mask_contains].copy()
df_date_external

# %%
df_date_external['Label'].unique()

# %%
df_date_external['Incubation protein source'].unique()

# %%
df_date_external['Protein source organism'].unique()

# %%
mask_external_plasma = df_date_external['Incubation protein source'].str.contains(
    'plasma', case=False, na=False
)
mask_external_human = df_date_external['Protein source organism'].str.contains(
    'human', case=False, na=False
)

df_date_external_plasma_human = df_date_external[mask_external_plasma & mask_external_human]

print(df_date_external_plasma_human['Protein source organism'].unique())
print(f"筛选到 {len(df_date_external_plasma_human)} 条记录。")

# %%
df_date_external_plasma_human = df_date_external_plasma_human[df_date_external_plasma_human['Imputation confidence']==1]
print(df_date_external_plasma_human['Protein source organism'].unique())
print(f"筛选到 {len(df_date_external_plasma_human)} 条记录。")
...
for doi in external_doi_list:
    # 对单个 doi 做 contains 搜索
    mask = df_date_external_plasma_human['Label'].str.contains(
        re.escape(doi), case=False, na=False
    )
    sub = df_date_external_plasma_human[mask]

    total = len(sub)
    count_1 = (sub['Protein corona composition'] == 1).sum()
    count_0 = (sub['Protein corona composition'] == 0).sum()

    rows.append([doi, total, count_1, count_0])

df_stats = pd.DataFrame(rows, columns=['doi', 'total_samples', 'count_1', 'count_0'])
df_stats


# %%
df_date_external_plasma_human.to_csv("data/date_external_plasma_human.csv", index=False)


# %%
# 按反斜杠分割，然后找出每行中包含 '10.' 的部分
doi_series = (
    df_date_external_plasma_human['Label']
    .str.split('\\')                                   # 分割路径
...
# %%
# 3. 使用布尔反转 (~) 找出所有【不包含】任一 DOI 的行
df = df[~mask_contains].copy() # 使用 .copy() 避免 SettingWithCopyWarning
df

...
# %%
metrics = mt
keys_to_keep = ['AUROC', 'AUPRC', 'F1', 'ACC']
metrics_save = {key: metrics[key] for key in keys_to_keep if key in metrics}

with open('explain_two_baseline.json', 'w') as f:
    json.dump(metrics_save, f, indent=4)

# %%
feature_rank = pd.read_csv("f1_diff_single.csv")
feature_rank
...
# %%
f1_result_df = pd.DataFrame(index=feature_columns, columns=feature_columns)
f1_result_df
...
for col1, col2 in feature_pairs:
    ...
    x_embed_data_non_fill = np.load(f"../text_embedding_4_explain/nonfill_two/text_embeddings_{cleaned_col1}_{cleaned_col2}.npy")
    ...
    current_f1 = results["metrics@thr"]["F1"]
    f1_result_df.loc[col1, col2] = current_f1

f1_result_df

# %%
f1_result_df.to_csv('tabular_feature_two.csv',)

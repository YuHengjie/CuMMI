...
if __name__ == '__main__':
    ...
    # 路径参数
    parser.add_argument("--save_dir", type=str, default="./output/basic", help="目录中通常包含 checkpoint 与 history")
     # 阈值策略
    ...
    text_embed_data = np.load("../../text_embedding/text_embeddings_nonfill.npy")  
    ...
    # --------- 准备测试集/外部集与 DataLoader（只构建一次）---------
    test_in_df  = pd.read_csv("data/basic_plasma_human_high_test.csv", keep_default_na=False, na_values=[''])
    test_out_df = pd.read_csv("data/protein_external_plasma_human_test.csv", keep_default_na=False, na_values=[''])
    ...
    for sub in subdirs:
        ...
        for load_stage in stage_nums:
            ...
            # 外部集
            results_out = evaluate_on_loader(model, test_out_loader, thr_history)
            #print("\n========== External Test ==========")
            
            m025 = results_out["metrics@0.25"]
            ...
            mb = results_out["metrics@best_on_test"]
            ...
            sub_summary[str(load_stage)] = {
                ...
                "external": {
                    "at_0p25": _to_py(results_out["metrics@0.25"]),
                    "at_0p5": _to_py(results_out["metrics@0.5"]),
                    "at_thr": _to_py(results_out["metrics@thr"]),
                    "best_on_test": _to_py(results_out["metrics@best_on_test"]),
                }
            }
    ...
    for sub in sorted(os.listdir(root_dir)):
        ...
        for stage, content in data.items():
            score_thr = content["external"]["at_thr"]
    ...
    # 按 external at_thr 的 Mean_Score (F1&AUROC) 排序
    df_sorted = df_summary.sort_values(by="Mean_Score (F1&AUROC)", ascending=False)

    print("\n===== Sorted by at_thr Mean_Score (F1&AUROC) =====")
    print(df_sorted.head(20))

    # 保存成 CSV
    output_csv = "eval_summary_external.csv"
